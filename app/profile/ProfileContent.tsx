...
import { fetchAllUserPosts } from "../utils/actions";
...
const ProfileContent = ({ initialUser, initialDbUser, initialPosts }: ProfileContentProps) => {
  const [showAllPosts, setShowAllPosts] = useState(false);
  const [posts, setPosts] = useState<any[]>([]);
  const [latestPosts] = useState<any[]>(initialPosts);

  React.useEffect(() => {
    const loadAllPosts = async () => {
      if (showAllPosts && initialDbUser) {
        const allPosts = await fetchAllUserPosts(initialDbUser.id);
        setPosts(allPosts);
      }
    };
    loadAllPosts();
  }, [showAllPosts, initialDbUser]);

  const displayPosts = showAllPosts ? posts : latestPosts;
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-2xl font-bold dark:text-white">
                {initialUser.lastName} {initialUser.firstName}
              </h1>
              <p className="text-gray-600 dark:text-gray-400">
                {initialUser.username}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Member since {new Date(initialDbUser.date).toLocaleDateString()}
              </p>
            </div>
          </div>
          <Link
